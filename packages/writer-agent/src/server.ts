...
async function main() {
  ...
  app.log.info('Writer agent initialized successfully');
  ...
  app.post<{ Body: ProcessRequest }>('/api/v1/writer/process', async (request, reply) => {
    try {
      const input = processRequestSchema.parse(request.body);
      app.log.info({ query: input.query }, 'Processing writer request');
  ...
  try {
    ...
    console.log(`✍️  Writer Agent running on http://${HOST}:${PORT}`);
    console.log(`   Health: http://${HOST}:${PORT}/health`);
    console.log(`   Metrics: http://${HOST}:${PORT}/metrics`);
    console.log(`   API: POST http://${HOST}:${PORT}/api/v1/writer/process`);
  } catch (err) {
    app.log.error(err);
    process.exit(1);
  }
...
main().catch((error) => {
  console.error('Failed to start Writer Agent:', error);
  process.exit(1);
});
